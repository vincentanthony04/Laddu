...
check('Current cache marker is terminal UI R3 E2E', 'terminal-ui-r3-e2e' in html and 'data-build-marker="terminal-actionable-ui-r3-e2e-acceptance"' in html)
...
check('Frontend identity failure is fail-closed for admission', 'frontendIdentityValid: false' in js and 'function customerTrust' in js and "state.frontendIdentityValid !== true" in js)
check('Actionable DOM exposes exact decision lineage for acceptance', 'data-decision-id=' in js and 'data-final-authority=' in js)
check('Browser identity marker must exactly match endpoint identity', "marker===expectedMarker" in js and "IDENTITY FAIL" in js)
print(json.dumps({'contract':'terminal-actionable-ui-r3-e2e-acceptance','pass':len(checks)-len(failed),'fail':len(failed),'checks':checks},indent=2))
